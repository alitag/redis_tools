...
/**
 * Redis的辅助类,负责对内存数据库的所有操作;采用切片化连接池的方式
 * 
 * @author gchangyi
 * @version 1.0
 */
public abstract class RedisExecutor {
	...
	/** 用于持有各种参数,将在构造函数中被初始化。 */
	private RedisConfig redisConfig;
	/** 数据切片连接池,通过一致性哈希算法决定把数据存到哪个redis实例上 */
	...
	public RedisExecutor(RedisConfig redisConfig) {
		...
		List<JedisShardInfo> shards = redisConfig.getShards();
		// 根据配置,创建shared池实例
		this.shardedJedisPool = new ShardedJedisPool(jedisPoolConfig, shards);
	}

	/**
	 * 执行器, 它保证在执行操作之后释放数据源jedis.close()
	 */
	abstract class Executor<T> {
		...
		/**
		 * 调用{@link #execute()}并返回执行结果 它保证在执行{@link #execute()}之后释放数据源shardedJedis.close()
		 * 
		 * @return 执行结果
		 */
	 ...
	 * 
	 * @param patten
	 *            模糊匹配key的表达式(*表示匹配所有的key)
	 * @return 匹配的结果集合
	 */
	public Set<String> getKeysLike(final String patten) {
	 ...
	 * @return 删除成功的条数
	 */
	public long delKeysLike(final String patten) {
	...
	public Long delKey(final String key, final String... fileds) {
		return new Executor<Long>(shardedJedisPool) {
			...
			@Override
			Long execute() {
				return shardedJedis.hdel(key, fileds);
			}
		}.getResult();
	}
	...
	public String setObject(final String key, final Serializable value) {
		return new Executor<String>(shardedJedisPool) {
			...
			@Override
			String execute() {
				return shardedJedis.set(SafeEncoder.encode(key), SerializationUtils.serialize(value));
			}
		}.getResult();
	}
	...
	public String setObject(final String key, final Serializable value, final int expire) {
		return new Executor<String>(shardedJedisPool) {
			...
			@Override
			String execute() {
				return shardedJedis.setex(key.getBytes(), expire, SerializationUtils.serialize(value));
			}
		}.getResult();
	}

	/**
	 * 把field和value键值对与key关联
	 * 
	 * @param key
	 * @param field
	 * @param value
	...
	public Long hset(final String key, final String field, final Serializable value) {
		return new Executor<Long>(shardedJedisPool) {
			...
			@Override
			Long execute() {
				return shardedJedis.hset(key.getBytes(), field.getBytes(), SerializationUtils.serialize(value));
			}
		}.getResult();
	}

	/**
	 * 把field和value键值对与key关联,并将 key 的生存时间设为 expire (以秒为单位)
	 * 
	 * @param key
	 * @param field
	 * @param value
	...
	public Long hset(final String key, final String field, final Serializable value, final int expire) {
		return new Executor<Long>(shardedJedisPool) {
			...
			Long execute() {
				...
				Response<Long> result = pipeline.hset(key.getBytes(), field.getBytes(),
	...
	public <T extends Serializable> T get(final String key) {
		return new Executor<T>(shardedJedisPool) {
			...
			T execute() {
				Object result = null;
				byte[] retVal = shardedJedis.get(SafeEncoder.encode(key));
				if (null != retVal) {
					try {
						result = SerializationUtils.deserialize(retVal);
					} catch (Exception e) {
	...
	public Long listDel(final String key, final String value, final int index) {
		return new Executor<Long>(shardedJedisPool) {
			...
			@Override
			Long execute() {
				return shardedJedis.lrem(key, index, value);
			}
		}.getResult();
	}
	...
	public Long sadd(final String key, final byte[]... members) {
		return new Executor<Long>(shardedJedisPool) {
			...
			@Override
			Long execute() {
				return shardedJedis.sadd(key.getBytes(), members);
			}
		}.getResult();
	}
	...
	public Set<String> sinter(final String... keys) {
		return new Executor<Set<String>>(shardedJedisPool) {
			...
			@Override
			Set<String> execute() {
				Jedis _jedis = shardedJedis.getShard(keys[0]);
	...
	public Set<String> sunion(final String... keys) {
		return new Executor<Set<String>>(shardedJedisPool) {
			...
			@Override
			Set<String> execute() {
				Jedis jedis = shardedJedis.getShard(keys[0]);
	...
	public <T extends Serializable> T hgetObj(final String key, final String field) {
		return new Executor<T>(shardedJedisPool) {
			...
			T execute() {
				...
				byte[] retVal = shardedJedis.hget(SafeEncoder.encode(key), SafeEncoder.encode(field));
				if (null != retVal) {
					try {
						result = SerializationUtils.deserialize(retVal);
					} catch (Exception e) {
